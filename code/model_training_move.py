...
# Ver 1.0    Nov 14 2016                                         |
# Author: zxm                                                    |
# Summary: to train a new move selector by supervised learning   |
#          using processed source data                           |
#          need to change the piece type manually                |
# ---------------------------------------------------------------+
...
# paras
# 32? to be determined
W_conv1 = weight_varible([3, 3, 9, feature_layer_1])
b_conv1 = bias_variable([feature_layer_1])

# conv layer-1
x = tf.placeholder(tf.float32, [None, 810])
x_image = tf.reshape(x, [-1, 10, 9, 9])
...
for filename in os.listdir(path):
    ...
    for line in pgn:
        ...
        if count % 10000 == 9999:
            saver.save(sess, 'my-model-n-', global_step = (count+1)/2)
